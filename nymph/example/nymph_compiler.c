...
char *protoCreate(char *token, FILE *outputHFP, struct dict **myDict, int *myDictLen, struct functions **functions, int *functionsLength) {
    ...
    char *protoDec = subString(token, ' ');
    token += strlen(protoDec) + 1;
